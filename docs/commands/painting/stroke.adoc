...
[`stroke`](stroke.adoc) paints a line following the current path using the current color. This line is centered on the path, has sides parallel to the path segments, and has a width (thickness) given by the current line width parameter in the graphics state.

[`stroke`](stroke.adoc) paints the joints between connected path segments with the current line join and the ends of open subpaths with the current line cap. The line is either solid or broken according to the dash pattern established by xref:../graphics-state/setdash.adoc[`setdash`].

The parameters in the graphics state controlling line rendition (line width, line join, line cap, dash pattern, etc.) are consulted at the time [`stroke`](stroke.adoc) is executed. Their values during path construction are irrelevant.

[`stroke`](stroke.adoc) implicitly performs a xref:../path-construction/newpath.adoc[`newpath`] after it has finished painting the current path.
...
WARNING: *Path Consumed After Stroke* - [`stroke`](stroke.adoc) clears the current path. Use xref:../graphics-state/gsave.adoc[`gsave`]/xref:../graphics-state/grestore.adoc[`grestore`] to preserve it.
...
[`stroke`](stroke.adoc) is affected by:
