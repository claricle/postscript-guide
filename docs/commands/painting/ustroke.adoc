...
[`ustroke`](ustroke.adoc) interprets a user path definition and strokes the resulting path as if by xref:stroke.adoc[`stroke`]. The entire operation is effectively enclosed by xref:../graphics-state/gsave.adoc[`gsave`] and xref:../graphics-state/grestore.adoc[`grestore`], so [`ustroke`](ustroke.adoc) has no lasting effect on the graphics state.

In the first form (with no matrix operand), [`ustroke`](ustroke.adoc) is equivalent to:
...
In the second form, [`ustroke`](ustroke.adoc) concatenates `matrix` to the CTM after interpreting `userpath`, but before executing xref:stroke.adoc[`stroke`]. The matrix applies to the line width and the dash pattern, if any, but not to the path itself. This form is equivalent to:
...
WARNING: *Graphics State Not Modified* - [`ustroke`](ustroke.adoc) automatically saves and restores graphics state.
...
[`ustroke`](ustroke.adoc) uses these graphics state parameters:
...
.User path approach ([`ustroke`](ustroke.adoc))
[source,postscript]
----
2 setlinewidth
[
