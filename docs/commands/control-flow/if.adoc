...
[`if`](if.adoc) removes both operands from the stack, then executes _proc_ if _bool_ is `true`. If _bool_ is `false`, _proc_ is not executed.

The [`if`](if.adoc) operator itself pushes no results, but _proc_ may push results onto the stack.
...
TIP: *Use ifelse for Two-Way* - For true/false cases, xref:ifelse.adoc[`ifelse`] is more convenient than nested [`if`](if.adoc) statements.
