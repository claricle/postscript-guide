...
link:quit.adoc[`quit`] terminates operation of the PostScript interpreter. The precise action depends on the environment:
...
In job server environments, link:quit.adoc[`quit`] in `userdict` is usually redefined to mean xref:stop.adoc[`stop`] (terminate job, not interpreter).
...
WARNING: *Immediate Termination* - link:quit.adoc[`quit`] terminates immediately, no cleanup.
...
TIP: *Use stop in Jobs* - In encapsulated jobs, use xref:stop.adoc[`stop`] instead of link:quit.adoc[`quit`].
