...
link:known.adoc[`known`] returns `true` if there is an entry in dictionary _dict_ whose key is _key_. Otherwise, it returns `false`.
...
WARNING: *Checks Specific Dictionary Only* - link:known.adoc[`known`] does NOT search the dictionary stack, only the specified dictionary.
...
WARNING: *Different from where* - link:known.adoc[`known`] checks one dictionary; xref:../where.adoc[`where`] searches the entire dictionary stack.
