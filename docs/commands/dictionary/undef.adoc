...
link:undef.adoc[`undef`] removes _key_ and its associated value from dictionary _dict_. The dictionary does not need to be on the dictionary stack.
...
WARNING: *Level 2 Only* - link:undef.adoc[`undef`] is not available in Level 1 implementations.
...
WARNING: *Silent on Missing Keys* - No error if key doesn't exist. Use link:known.adoc[`known`] first if you need to know.
