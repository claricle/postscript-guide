...
[`def`](def.adoc) associates _key_ with _value_ in the current dictionary (the one on top of the dictionary stack).
...
WARNING: *Defines in Current Dictionary* - [`def`](def.adoc) always uses the current dictionary (top of dict stack), not a specific dictionary.
