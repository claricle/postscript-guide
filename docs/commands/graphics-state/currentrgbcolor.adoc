...
[`currentrgbcolor`](currentrgbcolor.adoc) returns the current color in the graphics state according to the red-green-blue color model.

* If the current color space is **DeviceRGB**, [`currentrgbcolor`](currentrgbcolor.adoc) returns the color values most recently specified to [`setrgbcolor`](setrgbcolor.adoc) or [`setcolor`] (or transformed values specified to [`sethsbcolor`](sethsbcolor.adoc)).
* If the current color space is **DeviceGray** or **DeviceCMYK**, [`currentrgbcolor`](currentrgbcolor.adoc) converts the current color to RGB.
* For any other color space, [`currentrgbcolor`](currentrgbcolor.adoc) returns **0.0 0.0 0.0** (black).
