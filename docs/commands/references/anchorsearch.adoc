...
link:/commands/references/anchorsearch/[`anchorsearch`] determines if _string_ begins with _seek_ (i.e., _string_ is at least as long as _seek_ and the initial characters match exactly).
...
WARNING: *Must Match At Start* - link:/commands/references/anchorsearch/[`anchorsearch`] only finds prefixes, not substrings anywhere in the string.
...
TIP: *Use search for Anywhere* - For substring search anywhere in string, use link:/commands/references/search/[`search`] instead.
...
* link:/commands/references/search/[`search`] - Search for substring anywhere
* link:/commands/references/token/[`token`] - Parse token from string
* link:/commands/references/getinterval/[`getinterval`] - Extract substring