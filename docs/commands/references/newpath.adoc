...
link:/commands/references/newpath/[`newpath`] initializes the current path to be empty, causing the current point to become undefined.
...
WARNING: *No Current Point After newpath* - After link:/commands/references/newpath/[`newpath`], you must use link:/commands/references/moveto/[`moveto`] before link:/commands/references/lineto/[`lineto`], link:/commands/references/curveto/[`curveto`], etc.
...
WARNING: *Painting Operators Don't Clear Path* - Unlike painting operators in some graphics systems, PostScript's `stroke` and `fill` do not automatically clear the path. Use link:/commands/references/newpath/[`newpath`] explicitly.
...
TIP: *Use at Start of Path Procedures* - Always begin path construction procedures with link:/commands/references/newpath/[`newpath`] to ensure a clean slate.

=== Error Conditions

None. link:/commands/references/newpath/[`newpath`] cannot generate errors.
...
* Idempotent: calling link:/commands/references/newpath/[`newpath`] multiple times has same effect as calling once

=== Interaction with Graphics State

link:/commands/references/newpath/[`newpath`] only affects the current path. It does not change:
