...
link:/commands/references/forall/[`forall`] enumerates elements of the first operand, executing _proc_ for each:
...
If the composite object is empty (length 0), link:/commands/references/forall/[`forall`] does not execute _proc_ at all.
...
WARNING: *Modifying During Iteration* - Adding/removing dictionary entries during link:/commands/references/forall/[`forall`] may or may not affect iteration.
...
link:/commands/references/forall/[`forall`] is simpler and more efficient.

=== See Also

* link:/commands/references/aload/[`aload`] - Load all elements onto stack
* link:/commands/references/get/[`get`] - Get single element
* link:/commands/references/length/[`length`] - Get number of elements
