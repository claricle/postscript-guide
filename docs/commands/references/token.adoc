...
link:/commands/references/token/[`token`] reads characters from _file_ or _string_, interpreting them according to PostScript syntax rules until a complete object is scanned.
...
WARNING: *Consumes Whitespace* - link:/commands/references/token/[`token`] consumes trailing whitespace after names/numbers.
...
TIP: *Binary Token Support* - link:/commands/references/token/[`token`] also handles binary tokens and binary object sequences.
...
link:/commands/references/token/[`token`] consumes characters differently based on token type:
...
* Returns literal objects (use link:/commands/references/cvx/[`cvx`]) if execution needed
...
* link:/commands/references/search/[`search`] - Find substring
* link:/commands/references/anchorsearch/[`anchorsearch`] - Test for prefix
* link:/commands/references/cvs/[`cvs`] - Convert to string
* link:/commands/references/cvn/[`cvn`] - Convert to name
