...
The `rand` operator generates and returns a pseudo-random integer in the range 0 to 2³¹ - 1 (2147483647). The sequence is deterministic and can be controlled using link:/commands/references/srand/[`srand`] to set the seed and link:/commands/references/rrand/[`rrand`] to get/restore the generator state.
