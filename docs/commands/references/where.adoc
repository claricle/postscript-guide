...
link:/commands/references/where/[`where`] searches for _key_ in each dictionary on the dictionary stack, starting with the topmost (current) dictionary.
...
WARNING: *Returns Dictionary, Not Value* - link:/commands/references/where/[`where`] returns the dictionary itself, not the value.
...
[cols="2,3,3"]
|===
| Aspect | link:/commands/references/known/[`known`] | link:/commands/references/where/[`where`]
...
* link:/commands/references/known/[`known`] - Check specific dictionary
...
* link:/commands/references/store/[`store`] - Store in dictionary stack
