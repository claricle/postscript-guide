...
link:/commands/references/load/[`load`] searches for _key_ in each dictionary on the dictionary stack, starting with the topmost (current) dictionary.

* If found: pushes the associated value
* If not found: executes [`undefined`] error

link:/commands/references/load/[`load`] looks up keys the same way the interpreter looks up executable names, but always pushes the value (never executes it).
...
WARNING: *Always Pushes Value* - link:/commands/references/load/[`load`] never executes values, even executable ones.
...
[cols="2,3,3"]
|===
| Operation | link:/commands/references/load/[`load`] | link:/commands/references/get/[`get`]
...
* link:/commands/references/store/[`store`] - Store value in dictionary stack
...
* link:/commands/references/get/[`get`] - Get from specific dictionary
* link:/commands/references/known/[`known`] - Check if key exists