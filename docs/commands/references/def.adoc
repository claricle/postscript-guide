...
link:/commands/references/def/[`def`] associates _key_ with _value_ in the current dictionary (the one on top of the dictionary stack).
...
WARNING: *Defines in Current Dictionary* - link:/commands/references/def/[`def`] always uses the current dictionary (top of dict stack), not a specific dictionary.
...
* link:/commands/references/load/[`load`] - Look up value from dictionary stack
* link:/commands/references/store/[`store`] - Store with different semantics
* link:/commands/references/put/[`put`] - Put into specific dictionary
* link:/commands/references/begin/[`begin`] - Make dictionary current
