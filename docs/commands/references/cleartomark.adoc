...
This operator is commonly used to clean up after variable-argument operations or to discard intermediate results that were accumulated after placing a mark. It's the complement to link:/commands/references/mark/[`mark`] for managing stack segments.
...
TIP: Always pair link:/commands/references/mark/[`mark`] with `cleartomark` to avoid accumulating marks on the stack. Verify mark existence before calling `cleartomark` in defensive code.
...
* link:/commands/references/mark/[`mark`] - Push a mark object onto stack
...
1. **Always pair with mark**: Every link:/commands/references/mark/[`mark`] should have a matching `cleartomark` or `]`
...
* link:/usage/error-handling/[Error Handling] - Using marks for cleanup
