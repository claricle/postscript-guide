...
[`lineto`](lineto.adoc) appends a straight line segment to the current path. The line extends from the current point to the point (x, y) in user space; (x, y) then becomes the new current point.

If the current point is undefined because the current path is empty, [`lineto`](lineto.adoc) executes the [`nocurrentpoint`] error.
...
WARNING: *No Current Point* - [`lineto`](lineto.adoc) requires an established current point. Always use xref:moveto.adoc[`moveto`] first.
...
* Multiple consecutive [`lineto`](lineto.adoc) operations create a polyline
