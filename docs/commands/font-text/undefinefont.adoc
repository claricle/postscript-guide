...
link:undefinefont.adoc[`undefinefont`] removes `key` and its associated value (a font dictionary) from the font directory, reversing the effect of a previous link:definefont.adoc[`definefont`]. link:undefinefont.adoc[`undefinefont`] is a special case of the `undefineresource` operator applied to the Font category.

If the specified `key` does not exist in the font directory, link:undefinefont.adoc[`undefinefont`] does nothing—no error occurs.
...
**Current Font:**
- If the removed font is currently set via link:setfont.adoc[`setfont`], it remains current
- The current font in graphics state is unaffected
- Only future link:findfont.adoc[`findfont`] calls are affected

**Restore Interaction:**
- Effect of link:undefinefont.adoc[`undefinefont`] on local VM subject to `restore`
...
link:undefinefont.adoc[`undefinefont`] is equivalent to:
