...
link:currentfont.adoc[`currentfont`] returns the current font dictionary in the graphics state. Normally, this is the font most recently established by link:setfont.adoc[`setfont`] or `selectfont`.

However, when executed inside a font's `BuildGlyph` or `BuildChar` procedure or a procedure invoked by link:cshow.adoc[`cshow`], link:currentfont.adoc[`currentfont`] returns the currently selected base font (descendant of a composite font).
...
When link:currentfont.adoc[`currentfont`] is called during font construction:

**In BuildGlyph/BuildChar procedures:**
: Returns the base font selected by the composite font machinery

**In link:cshow.adoc[`cshow`] procedure:**
: Returns the base font for the current character

**Normal context:**
: Returns the top-level font set by link:setfont.adoc[`setfont`]
