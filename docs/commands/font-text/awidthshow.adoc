...
link:awidthshow.adoc[`awidthshow`] paints the characters of `string` in a manner similar to link:show.adoc[`show`], but combines the special effects of link:ashow.adoc[`ashow`] and link:widthshow.adoc[`widthshow`].

link:awidthshow.adoc[`awidthshow`] adjusts the width of each character shown by adding `ax` to its x width and `ay` to its y width, thus modifying the spacing between characters. Furthermore, link:awidthshow.adoc[`awidthshow`] modifies the width of each occurrence of the character `char` by an additional amount `(cx, cy)`.

The interpretation of `char` is as described for the link:widthshow.adoc[`widthshow`] operator.
...
- More expensive than link:show.adoc[`show`], link:ashow.adoc[`ashow`], or link:widthshow.adoc[`widthshow`] alone
