...
link:widthshow.adoc[`widthshow`] paints the characters of `string` in a manner similar to link:show.adoc[`show`]. But while doing so, link:widthshow.adoc[`widthshow`] adjusts the width of each occurrence of the character `char` by adding `cx` to its x width and `cy` to its y width, thus modifying the spacing between it and the next character.
...
- Slightly slower than link:show.adoc[`show`] due to conditional width adjustment
- Still benefits from font caching
- More efficient than link:kshow.adoc[`kshow`] for simple spacing needs
...
- link:ashow.adoc[`ashow`] - Show with uniform character spacing
