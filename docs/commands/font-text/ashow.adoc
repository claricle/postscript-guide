...
link:ashow.adoc[`ashow`] paints the characters of `string` in a manner similar to link:show.adoc[`show`]. But while doing so, link:ashow.adoc[`ashow`] adjusts the width of each character shown by adding `ax` to the character's x width and `ay` to its y width, thus modifying the spacing between characters.
...
**nocurrentpoint**
: Current point is not defined before link:ashow.adoc[`ashow`]
...
- link:ashow.adoc[`ashow`] is slightly slower than link:show.adoc[`show`] due to spacing calculations
- Character caching still applies
- For uniform spacing of all text, set once and use link:show.adoc[`show`]
- For spacing only specific characters, use link:widthshow.adoc[`widthshow`] instead
...
- link:widthshow.adoc[`widthshow`] - Adjust specific character width
- link:awidthshow.adoc[`awidthshow`] - Combine link:ashow.adoc[`ashow`] and link:widthshow.adoc[`widthshow`]
