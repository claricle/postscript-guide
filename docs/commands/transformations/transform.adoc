...
With no matrix operand, link:transform.adoc[`transform`] transforms the user space coordinate (x, y) by the current transformation matrix (CTM) to produce the corresponding device space coordinate (x', y').

If the `matrix` operand is supplied, link:transform.adoc[`transform`] transforms (x, y) by `matrix` rather than by the CTM.
...
WARNING: *Position-Dependent* - link:transform.adoc[`transform`] includes translation. Use xref:dtransform.adoc[`dtransform`] for distances.
...
TIP: *Use for Debugging* - link:transform.adoc[`transform`] helps understand coordinate mappings:
