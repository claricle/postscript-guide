...
With no matrix operand, [`dtransform`](dtransform.adoc) (delta transform) transforms the distance vector (dx, dy) by the CTM to produce the corresponding distance vector (dx', dy') in device space.

If the `matrix` operand is supplied, [`dtransform`](dtransform.adoc) transforms the distance vector by `matrix` rather than by the CTM.
...
WARNING: *Position-Independent* - [`dtransform`](dtransform.adoc) ignores translation. Use xref:transform.adoc[`transform`] for points.
