...
link:aload.adoc[`aload`] successively pushes all _n_ elements of an array or packed array onto the operand stack (where _n_ is the length of the operand), and finally pushes the array or packed array itself back onto the stack.
...
WARNING: *Array Remains on Stack* - link:aload.adoc[`aload`] leaves the original array on the stack. Use link:../stack-manipulation/pop.adoc[`pop`] if you don't need it.
...
TIP: *Use with forall* - For processing array elements, xref:forall.adoc[`forall`] is often more appropriate than link:aload.adoc[`aload`].
