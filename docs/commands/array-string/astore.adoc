...
[`astore`](astore.adoc) stores objects from the operand stack into an array:
...
The array must have been previously created with the correct length. [`astore`](astore.adoc) cannot be performed on packed arrays (they are read-only).
...
The `[ ]` syntax is usually more convenient for literal arrays, but [`astore`](astore.adoc) is useful when the array size is computed dynamically.
