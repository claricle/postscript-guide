...
[`wcheck`](wcheck.adoc) tests whether the operand's access attribute permits its value to be written (modified) by PostScript operators.
...
WARNING: *Packed Arrays Always Read-Only* - Packed arrays created with xref:packedarray.adoc[`packedarray`] always return `false` for [`wcheck`](wcheck.adoc).
...
WARNING: *Confusing wcheck with xcheck* - [`wcheck`](wcheck.adoc) tests write access, xref:xcheck.adoc[`xcheck`] tests executable attribute.

TIP: *Check Before Modifying* - Use [`wcheck`](wcheck.adoc) before operations like xref:put.adoc[`put`] or xref:putinterval.adoc[`putinterval`] on untrusted objects.
