...
[`anchorsearch`](anchorsearch.adoc) determines if _string_ begins with _seek_ (i.e., _string_ is at least as long as _seek_ and the initial characters match exactly).
...
WARNING: *Must Match At Start* - [`anchorsearch`](anchorsearch.adoc) only finds prefixes, not substrings anywhere in the string.
